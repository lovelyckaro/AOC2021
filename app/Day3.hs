...
mostCommon n xs = 
  let bits = map (`testBit` n) xs in
    if length (filter id bits) >= length (filter not bits)
      then bit n
      else zeroBits
...
oxygen = head . go 11
  where
    ...
    go bit xs = go (bit - 1) (filter (\word -> testBit word bit == testBit wanted bit) xs)
      where wanted = mostCommon bit xs
...
co2 = head . go 11
  where
    ...
    go _ [x] = [x]
    go bit xs = go (bit - 1) (filter (\word -> testBit word bit == testBit wanted bit) xs)
      where wanted = leastCommon bit xs

part2 :: String -> Int
part2 inp = co2 xs * oxygen xs
  where xs = readInp inp
